...
configure-codespaces-keycloak: | $(KUBECTL) ## Configure Keycloak realm rootUrl for GitHub Codespaces
	@if [ -n "$(CODESPACE_NAME)" ] && [ -n "$(GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN)" ]; then \
		...
		echo "--> INFO: Configuring Keycloak realm rootUrl for Codespaces..." ;\
		...
		$(KUBECTL) exec -n $(EDA_CORE_NAMESPACE) deploy/eda-toolbox -- sh -c "curl -skf \
			...
			| jq '.attributes.rootUrl = \"$${CODESPACE_URL}/core/proxy/v1/identity\"' \
			| curl -skf -X PUT \
				'$${KC_URL}/admin/realms/eda' \
				-H 'Authorization: Bearer $${TOKEN}' \
				-H 'Content-Type: application/json' \
				--data @-" ;\
		echo "--> INFO: Keycloak realm rootUrl set to: $${CODESPACE_URL}/core/proxy/v1/identity" ;\
	else \
		echo "--> INFO: Not running in Codespaces, skipping Keycloak realm rootUrl configuration" ;\
	fi
